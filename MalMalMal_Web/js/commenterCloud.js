...
//	Our namespace
	...
	Cloud.init = function(){
		...
		this.stage = new Kinetic.Stage({
			container: 'comenterCloud',
			width: $canvasArea.width(),
			height: $canvasArea.height()
		});
		NOW_CANVAS = this.stage;
		...
		//We build the commenterTags
		this.buildcommenterTags();

		//We draw the stage
		this.stage.draw();
		NOW_COMMENTER_CLOUD = this;
	};
...
//	Adjusting offsets to be at the center
	...
	Cloud.buildcommenterTags = function () {
		for (var item in commenterTags) {
			// We draw out items
			this.buildTag(commenterTags[item],item);
			// We launch the tweening animation
			this.moveAround(commenterTags[item]);
			this.addBindings(commenterTags[item]);
		}
	};
//http://61.43.139.19/WebService.asmx/IC0121
	...
	Cloud.addBindings =function(tag) {
		...
		tag.group.on('mouseenter',function(e){
			...
			if(tag.related[0]!="null"){
				...
				for(var i=0,len=tag.related.length;i<len;i++) {
					Cloud.drawJunction(tag,commenterTags[tag.related[i]]); // Building junction between 2 commenterTags
					Cloud.attract(tag,commenterTags[tag.related[i]]); // We create attraction
				}
				Cloud.repelUnrelated(tag);
			}
				var temp = "발언수 : "+tag.in_idx+"<br>";
		...
		tag.group.on('mouseleave',function(){
			...
			if(tag.related[0]!="null"){
				...
				for (var item in commenterTags) {
					Cloud.moveAround(commenterTags[item]);
				}
	...
	Cloud.repelUnrelated = function(tag) {
		...
		for(var i=0,len=unrelated.length;i<len;i++) {
			Cloud.repel(tag,commenterTags[unrelated[i]]);
		}
	};
	...
	Cloud.getUnrelated = function(tag){
		//This return an array of commenterTags name that are not related to the tag
		// ie: for "milk" it returns ["great", "elyx0"]
		return Object.keys(commenterTags).filter(function(i){
...
//		v
	...
	Cloud.getPadding= function(tag) {
		...
		if(tag.related[0]!="null"){
			for(var i=0,len=tag.related.length;i<len;i++) {
				if (commenterTags[tag.related[i]].group.getWidth()/2 > BOX_PADDING) {
					BOX_PADDING = commenterTags[tag.related[i]].group.getWidth()/2;
				}
			}
		}
	...
	Cloud.buildTag = function(tag,name){
		...
		var simpleText = new Kinetic.Text({
			...
			fontSize: 20*(1+tag.importance/6),
			name: name,
		...
		//We store the group object in the initial commenterTags object for further use
		tag.group = group;
